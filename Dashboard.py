...
def Start_dashboard():
    ...
    efficiency_metrics_frame.place(x=1, y=644, relx=0.004, rely=0.01)
    ...
    dmg_per_min_Label.grid(row=0, column=0, sticky="n", padx=5, pady=7)
    ...
    dmg_per_min_data.grid(row=0, column=1, sticky="n", padx=5, pady=7)
    ...
    gold_per_min_Label.grid(row=1, column=0, sticky="n", padx=5, pady=7)
    ...
    type_of_killing_Sizes = [Data_frame[0]['My_main_kill'].sum(),
                             Data_frame[0]['Kill_Steal'].sum(),
                             Data_frame[0]['Regular_kill'].sum(),
                             Data_frame[0]['Solo_kill'].sum()]
    ...
    type_of_death_frame = tk.LabelFrame(root, text="Type of Deaths", bg="#333333", fg="#FFFFFF", labelanchor="n")
    type_of_death_frame.place(x=620, y=156, relx=0.15, rely=0.01)

    type_of_death_label = ["Solo death", "Adc & supp", "MyL & Jun", "Team fight"]
    type_of_death_sizes = [
        Data_frame[0]['Solo_death'].sum(),
        Data_frame[0]['Adc_supp'].sum(),
        Data_frame[0]['My_lane_and_jungler'].sum(),
        Data_frame[0]['Team_fight'].sum()
    ]

    filtered_labels2 = [label for label, size in zip(type_of_death_label, type_of_death_sizes) if size != 0]
    filtered_sizes2 = [size for size in type_of_death_sizes if size != 0]

    fig2 = plt.figure(figsize=(3, 2.5), dpi=100)
    fig2.patch.set_facecolor("#333333")
    plt.rcParams['text.color'] = '#FFFFFF'

    plt.pie(filtered_sizes2, labels=filtered_labels2, autopct='%.0f%%')

    canvas_type_of_death = FigureCanvasTkAgg(fig2, master=type_of_death_frame)
    canvas_type_of_death.draw()
    canvas_type_of_death.get_tk_widget().pack()
