...
key = "RGAPI-b9d8c141-3051-425d-83ed-77990ee20902"
...
def get_player_roles(match_data):
    roles = {}
...
def type_of_death(time_line_data_per_game, player_index, roles):
    solo_death = 0  # 100% DMG from a single source
    adc_supp = 0  # 2 champions only (adc and support) participate in my death
    my_lane_and_jungler = 0  # my opponent in my lane and jungler participate in my death
    team_fight = 0  # 3 or more champions participate in my death

    player_role = roles[player_index + 1]
    lane_opponent_role = {
        "TOP": "TOP",
        "JUNGLE": "JUNGLE",
        "MIDDLE": "MIDDLE",
        "BOTTOM": "BOTTOM",
        "UTILITY": "UTILITY"
    }.get(player_role, "UNKNOWN")

    for frame in time_line_data_per_game["info"]["frames"]:
        for event in frame["events"]:
            if event.get("type") == "CHAMPION_KILL" and event.get("victimId") == player_index + 1:
                dmg_sources = event.get("victimDamageReceived", [])
                participants = {dmg.get("participantId") for dmg in dmg_sources if dmg.get("participantId")}

                if len(participants) == 1:
                    solo_death += 1
                elif len(participants) == 2:
                    roles_involved = {roles[participant] for participant in participants}
                    if roles_involved == {"BOTTOM", "UTILITY"}:
                        adc_supp += 1
                    elif roles_involved == {lane_opponent_role, "JUNGLE"}:
                        my_lane_and_jungler += 1
                elif len(participants) >= 3:
                    team_fight += 1
    return solo_death, adc_supp, my_lane_and_jungler, team_fight
...
def process_match(Regoins_m, key, match, player_puuid):
    ...
    roles = get_player_roles(match_data)
    deaths_data = type_of_death(time_line_data_per_game, player_index, roles)
    ...
    return {
        ...
        **k_d_a_time_intervals,
        ...
        "Solo_death": deaths_data[0],
        "Adc_supp": deaths_data[1],
        "My_lane_and_jungler": deaths_data[2],
        "Team_fight": deaths_data[3]
    }
...
def Main(matches_list, Regoins_m, player_puuid, key):
    ...
    Main_frame = pd.DataFrame(columns=[
        ...
        "KDA_per_min", "Dmg_per_min", "Gold_per_min", "CS_diff_at_10", "XP_diff_at_10", "Gold_diff_at_10","CS_per_min",
        "Solo_death", "Adc_supp","My_lane_and_jungler", "Team_fight"
    ])
